...
export default function Report() {
  ...
  const capKWp =
    Number(
      String(
        (confirmData?.installedCapacity != null
          ? `${confirmData.installedCapacity}`
          : installed) || ""
      ).replace(/[^\d.]/g, "")
    ) || 0;
  ...
  const getFontSizeForInverter = (text = "") => {
    ...
    if (isMobile) {
      ...
      if (len > 45) return "12.5px";
                <p className="font-semibold">{siteName || confirmData?.siteName || "Unnamed Site"}</p>
      return "15px";
    }
                  <span className="font-medium text-white">{cod || confirmData?.codDate || "--"}</span>
          ...
          <div className="text-left md:text-right text-xs text-gray-100 leading-5 mt-3 md:mt-1">
            ...
            <p>
              ...
              <span className="font-medium text-white">{generatedText}</span>
                {installed || (confirmData?.installedCapacity ? `${confirmData.installedCapacity} kWp` : "—")}
          </div>
        </div>
        ...
        <div className="w-full max-w-[850px] bg-white rounded-lg border border-gray-200 shadow-sm p-5 mt-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
            <div>
              ...
              <p className="font-medium text-gray-800">{siteName || confirmData?.siteName || "—"}</p>
            </div>
            <div>
              <p className="text-gray-500">Installed Capacity</p>
              <p className="font-medium text-gray-800">{installed || (confirmData?.installedCapacity ? `${confirmData.installedCapacity} kWp` : "—")}</p>
            </div>
            <div>
              <p className="text-gray-500">COD</p>
              <p className="font-medium text-gray-800">{cod || confirmData?.codDate || "—"}</p>
            </div>
          </div>
        </div>
              🔧 PV / INV
                  ...
                  return (
                    <div className="bg-white min-h-screen flex flex-col items-center text-gray-800">
                      ...
                      <div className="w-full max-w-[850px] bg-[#2563EB] rounded-lg shadow-md px-6 md:px-10 py-6 text-white">
                        <div className="flex flex-col md:flex-row md:justify-between md:items-start mb-3">
                          <div>
                            ...
                            <div className="flex flex-wrap items-center gap-2 text-sm md:text-base">
                              ...
                              <p className="text-gray-100">
                                • COD:{" "}
                                <span className="font-medium text-white">{confirmData?.codDate || cod || "--"}</span>
                              </p>
                            </div>
                          </div>
                          <div className="text-left md:text-right text-xs text-gray-100 leading-5 mt-3 md:mt-1">
                        ...
                        <div className="grid gap-3 md:gap-5 mt-2 grid-cols-1 sm:grid-cols-2 md:grid-cols-[0.9fr_1fr_1.6fr]">
                          ...
                          <div className="bg-white/10 border border-white/25 rounded-xl px-5 py-3">
                            ...
                            <p className="text-white font-semibold text-sm md:text-base">
                              {confirmData?.installedCapacity ? `${confirmData.installedCapacity} kWp` : (installed || "—")}
                            </p>
                          </div>
                          <div className="bg-white/10 border border-white/25 rounded-xl px-5 py-3">
                      ...
                      <div className="w-full max-w-[850px] bg-[#F9FBFF] rounded-lg shadow p-6 mb-6 mt-5">
                        ...
                        <div className="flex flex-col md:flex-row gap-6">
                          ...
                          <div className="flex-1 bg-[#FFFFFF] rounded-2xl shadow-sm p-4">
                            ...
                            <ResponsiveContainer width="100%" height={220}>
                              <LineChart
                                data={dailyRPR.length > 0 ? dailyRPR : [{ date: "01", RPR: Number(realPR) || 0 }]}
                        ...
                        {!irrFile && (
                          <p className="text-left text-gray-500 italic text-xs mt-3 ml-1">No irradiation file uploaded — using monthly baseline for reference.</p>
                        )}
                      </div>
